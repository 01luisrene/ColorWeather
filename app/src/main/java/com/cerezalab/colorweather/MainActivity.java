...
public class MainActivity extends Activity {
    ...
    public static final String MINUTE_ARRAYY_LIST = "MINUTE_ARRAYY_LIST";
    ...
    public void minutelyWeatherClick(){
        ...
        minutelyActivityIntent.putParcelableArrayListExtra(MINUTE_ARRAYY_LIST, minutes);
    ...
    private CurrentWeather getCurrentWeatherFromJson(String json) throws JSONException{
        ...
        JSONObject jsonWinthDailyWeather = jsonObject.getJSONObject(DAILY);
        JSONArray jsonWithDailyWeatherData = jsonWinthDailyWeather.getJSONArray(DATA);
        JSONObject jsonWidthTodayDate = jsonWithDailyWeatherData.getJSONObject(0);
        ...
        String maxTemperature = Math.round(jsonWidthTodayDate.getDouble(TEMPERATURE_MAX)) + "";

        String minTemperature = Math.round(jsonWidthTodayDate.getDouble(TEMPERATURE_MIN)) + "";
    ...
    public ArrayList<Hour> getHourlyWeatherFromJson(String json) throws JSONException{
        ...
        for (int i = 0; i < jsonWithHourlyWeatherData.length(); i++){
            ...
            JSONObject jsonWithHourDate = jsonWithHourlyWeatherData.getJSONObject(i);

            String title = dateFormat.format(jsonWithHourDate.getDouble(TIME)*1000);

            String description = jsonWithHourDate.getString(SUMMARY);
    ...
    public ArrayList<Minute> getMinutelyWeatherFromJson(String json) throws JSONException{
        ...
        JSONObject jsonWithMinuttelyWeather = jsonObject.getJSONObject(MINUTELY);

        JSONArray jsonWithMinutelyWeatherData = jsonWithMinuttelyWeather.getJSONArray(DATA);

        for (int i = 0; i < jsonWithMinutelyWeatherData.length(); i++){

            Minute minute = new Minute();

            JSONObject jsonWithMinuteDate = jsonWithMinutelyWeatherData.getJSONObject(i);

            String title = dateFormat.format(jsonWithMinuteDate.getDouble(TIME)*1000);

            String precipProbability = "Rain probability: "+jsonWithMinuteDate.getDouble(PRECIP_PROBABILITY)*100 + "%";
